...
| Field | Notes |
|---|---|
| **Class name** | The full class name including prefix, e.g. `AMyActor`. A warning appears if the name is missing the expected `A` prefix (Actor-derived) or `U` prefix (UObject-derived). |
...
You don't need to select a parent class. The **New Class** and **New Struct** buttons in the toolbar create a class or struct with no parent.
...
Struct generation also triggers automatically when the selected parent class is an `F`-prefixed type (e.g. `FTableRowBase`).

## Settings

Settings are saved to `%LOCALAPPDATA%\UEClassCreator\settings.json` and persist across sessions. **Company name** is set at the top of the right panel and appears in the copyright header of every generated file. The **UObject only** filter and **Open in Explorer after create** checkbox are also persisted globally.

Per-project state — last output path and last selected class — is keyed to the `.uproject` path and restored automatically when you switch projects.

## Update Notifications

At startup the tool checks the GitHub releases page. If a newer version is available, a banner appears at the top of the window. Click it to open the release page in your browser.
